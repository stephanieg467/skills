...
Transform a feature request into a **comprehensive implementation plan** through systematic codebase analysis, conditional external research, and strategic planning.
...
Preserve comprehensive evidence coverage without accumulating every discovery in the parent session:

- Start with `rg`/search to locate relevant symbols, callers, tests, configuration, and docs. Follow with targeted symbol or line-range reads. Read a complete file when it is small or its full lifecycle/contract is genuinely needed; never impose an arbitrary line limit that could truncate a function, type, or contract.
- For broad repository reconnaissance, use fresh, narrowly scoped subagents where beneficial (for example, one integration seam or test area each). Ask for concise, source-linked reports targeting 2–4K tokens. The parent must not repeat a subagent's full scan; verify only load-bearing claims and ranges used by the plan.
- Bound each model-facing tool-result batch by expected output—roughly 20–30K new tokens. Split large searches or reads by subsystem/file cluster, then record a settled phase summary before moving on so compaction can run without losing decisions or evidence.
...
Research externally only when a material implementation fact remains unresolved after checking inherited architecture, repository code, and local documentation. Use a fresh, narrowly scoped research subagent when beneficial, and prioritize primary/official sources over tutorials or commentary.

**Documentation Gathering (as needed):**

- Confirm only the relevant library/API behavior, supported versions, and compatibility constraints
- Find official documentation with specific section anchors
- Check official migration guides, security guidance, and known breaking changes when applicable
- Use third-party examples only when primary sources do not resolve the fact, and label their authority

Skip general technology-trend research that does not change the ticket-level plan.
...
### Relevant Codebase Files IMPORTANT: REVIEW THESE REFERENCES BEFORE IMPLEMENTING!

<List exact files and symbol/line ranges with relevance. Summarize the pattern or contract rather than embedding large source excerpts or requiring every referenced file to be read in full. The execution agent should expand to a complete file when it is small or full context is needed.>

- `path/to/file.py` (lines 15-45) - Why: Contains pattern for X that we'll mirror
- `path/to/model.py` (lines 100-120) - Why: Database model structure to follow
- `path/to/test.py` (`test_relevant_behavior`) - Why: Test pattern example
...
<Summarize specific patterns extracted from the codebase and cite exact files/ranges. Include only short source excerpts when wording alone cannot communicate a load-bearing detail.>
...
- [ ] All necessary patterns identified, summarized, and linked to exact files/ranges
- [ ] External library usage documented with primary/official links when external research was needed
